...
batch_size = int(input("How big should batches be ? (should be between 1000 and 25000) \n"))
    ...
    for line in lines:
        if(len(line.split()) == 2):
            ...
            if(batch_check >= batch_size):
                    batch_check =0
                    print(f"Batch {batch_count} has poz:{count_atomi[2]-batch_atomi[2]} // neg:{count_atomi[0]-batch_atomi[0]}")
                    batch_poz.append(count_atomi[2]-batch_atomi[2])
                    batch_neg.append(count_atomi[0]-batch_atomi[0])
                    for n in range(0,4):
                        batch_atomi[n] = count_atomi [n]
                    batch_count += 1
        else:
...
#graph_length = int(input("How many bars should the chart have?"))
x = range(min(len(batch_neg), len(batch_poz)))
plt.figure(figsize=(12,4))
plt.plot(x, batch_poz, color='blue', label='Positive')
plt.plot(x, batch_neg, color='red' , label='Negative')
plt.xlabel('Batch Number')
plt.ylabel('Count')
plt.title('Positive and Negative Particles per Batch (Trend)')
plt.legend()
plt.tight_layout()
plt.show()
